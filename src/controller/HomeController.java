...
public class HomeController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String urlString="https://api.themoviedb.org/3/movie/upcoming?api_key="+ key+"&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&primary_release_year=2016";
		URL url=new URL(urlString);
		String responsedata = IOUtils.toString(url);
		try{			
			List<Discover> discover=new ArrayList<Discover>();
			JSONObject output=(JSONObject)JSONValue.parseWithException(responsedata);
			...
			for(int i=0;i<results.size();i++)
			{
				...
				Discover d=new Discover("https://image.tmdb.org/t/p/w1280"+image, id, title);
				discover.add(d);
			}
